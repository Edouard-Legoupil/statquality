...
mod_mtcars_server <- function(input, output, session) {
  ...
  output$plot <- renderPlotly({
    ...
    p <- dt %>%
      ggplot(
        aes(x = eval(sym(input$cols)), 
            y = eval(sym(input$cols_dep)))
      ) +
     #  geom_point(color = unhcrthemes::unhcr_pal(n = 1, "pal_blue"),  size = 2.5) +
     geom_point(color = "orange", size = 2) +
      theme_bw() +
      #unhcrthemes::theme_unhcr(grid = "XY", axis = FALSE, axis_title = "xy") +
      labs(
        x = paste0(input$cols),
        y = paste0(input$cols_dep),
        title = paste0("Plot of ", input$cols, " by ", input$cols_dep),
